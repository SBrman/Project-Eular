...
data = ('08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 '#0
        ...
        '20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54 '#18   #19
...
data = data.split(' ')
data = [[int(data[j]) for j in range(i, i+COLUMN)] for i in range(0, ROW*COLUMN, ROW)]

def productOf(row, col, fixed):
    ...
    x,y = (max(-(ADJACENT_CHECKS-1)+index,0),
           min(index+(ADJACENT_CHECKS-1), (COLUMN-1)))
    loopNum = (y-x-(ADJACENT_CHECKS-1)) + 1
    pmax = 1                                 
    for i in range(loopNum):
        p = 1
        for k in range(x+i, (x+i+(ADJACENT_CHECKS-1))+1, 1): 
            if fixed == row:
                p *= data[fixed][k]
            elif fixed == col:
                p *= data[k][fixed]
...
def maxProduct(row, col):

    # Horizonral
    hmax = productOf(row, col, fixed=row)

    # Vertical
    vmax = productOf(row, col, fixed=col)

    # Diagonals
    pdmax = 1
    if (ADJACENT_CHECKS-1) <= row <= (COLUMN-ADJACENT_CHECKS) and \
       (ADJACENT_CHECKS-1) <= col <= (COLUMN-ADJACENT_CHECKS):

        l,r,u,d = (max(-(ADJACENT_CHECKS-1)+col,0),
                   min(col+(ADJACENT_CHECKS-1),(COLUMN-1)),
                   max(-(ADJACENT_CHECKS-1)+row,0),
                   min(row+(ADJACENT_CHECKS-1),(COLUMN-1)))

        lurdIndex = list(zip(range(l, r+1), range(u, d+1))) 
        ldruIndex = list(zip(range(l, r+1), range(d, u-1,-1)))

        for diagIndex in (lurdIndex, ldruIndex):
            for i in range(len(diagIndex)-(ADJACENT_CHECKS-1)):
                pd1 = 1
                for j in range(i, (i+(ADJACENT_CHECKS-1))+1):
                    r, c = diagIndex[j]
                    pd1 *= data[r][c]
                if pd1 > pdmax:
                    pdmax = pd1

    maxProduct = max(hmax, vmax, pdmax)
    return maxProduct

maxproduct = 1
for row in range(ROW):
    for col in range(COLUMN):
        product = maxProduct(row, col)
